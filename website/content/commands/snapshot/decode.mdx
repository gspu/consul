...
description: |
  The `consul snapshot decode` command outputs a stream of line-delimited JSON objects that contain an atomic point-in-time snapshot of the state of the Consul servers. The output includes registered services in the catalog, prepared queries, sessions, key/value entries, and ACLs.
---
...
The `snapshot decode` command accepts a file that contains a atomic point-in-time
snapshot of the state of the Consul servers as input and then decodes the snapshot 
to `stdout` as JSON data for interpretation and review. This snapshot includes the 
service catalog, prepared queries, sessions, key/value entries, and ACLs. 

Typical use of this command occurs with Consul self-contained snapshot files obtained
with the [`consul snapshot`](/consul/commands/snapshot) command or [Snapshot
API](/consul/api-docs/snapshot#generate-snapshot).

The `consul snapshot decode` command outputs JSON stream information in the following format:

```json hideClipboard
  {"Data": {"<json encoded snapshot header>"}, "Type": "SnapshotHeader",}
  {"Data": {<json encoded data>}, "Type": "<type name>"}
  {"Data": {<json encoded data>}, "Type": "<type name>"}
  ...
```
...
For more information about snapshots and how they work, refer to the [`snapshot/` HTTP API endpoint](/consul/api-docs/snapshot) documentation.
